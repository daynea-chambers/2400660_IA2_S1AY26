/*
 script.js — Inked Thoughts demo

 Purpose:
 - Handles registration/login UI behavior
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Generic data-action click handler: calls a global function named in data-action
    document.querySelectorAll('[data-action]').forEach(el => {
        const action = el.dataset.action;
        el.addEventListener('click', function(e) {
            // Prevent default for buttons inside forms when appropriate
            if (el.type === 'button') e.preventDefault();
    ...
    // Checkout button: call Checkout() before navigating (if present)
    ...
    // Attach other button handlers by id
    ...
    // Logout links (call Logout() if defined)
